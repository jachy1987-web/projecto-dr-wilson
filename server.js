...
const PORT = 3001;
const ESTADOS_CITA_VALIDOS = ["Programada", "Atendida", "Cancelada"];

function textoValido(valor, maximo = 500) {
...
function fechaHoraValida(valor) {
    return /^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/.test(String(valor || ""));
}
...
app.post("/api/citas", function(req, res) {
    ...
    if (!fechaHoraValida(fecha_hora) || !textoValido(motivo, 200) || !enteroPositivo(id_mascota) || !enteroPositivo(id_usuario) || (estado && !ESTADOS_CITA_VALIDOS.includes(estado))) {
        return res.status(400).json({ mensaje: "Verifique los datos obligatorios de la cita." });
    }
    ...
    conexion.query(
        "SELECT id_cita FROM citas WHERE fecha_hora = ? AND id_usuario = ? AND estado <> 'Cancelada' LIMIT 1",
        [fecha_hora, id_usuario],
        function(error, existentes) {
            if (error) { return res.status(500).json({ mensaje: "Error al validar la disponibilidad de la cita" }); }
            if (existentes.length > 0) { return res.status(409).json({ mensaje: "El veterinario ya tiene una cita programada en esa fecha y hora." }); }
...
app.put("/api/citas/:id", function(req, res) {
    ...
    if (!fechaHoraValida(fecha_hora) || !textoValido(motivo, 200) || !enteroPositivo(id_mascota) || !enteroPositivo(id_usuario) || !ESTADOS_CITA_VALIDOS.includes(estado)) {
        return res.status(400).json({ mensaje: "Verifique los datos obligatorios de la cita." });
    }
...
app.post("/api/historias-clinicas", function(req, res) {
    ...
    if (!fechaValida(fecha) || !textoValido(motivo_consulta, 200) || !enteroPositivo(id_mascota) || !enteroPositivo(id_usuario)) {
        return res.status(400).json({ mensaje: "Verifique la fecha, motivo, mascota y veterinario." });
    }
...
app.post("/api/vacunaciones", function(req, res) {
    ...
    if (!enteroPositivo(id_mascota) || !enteroPositivo(id_vacuna) || !fechaValida(fecha_aplicacion) || (proxima_dosis && !fechaValida(proxima_dosis))) {
        return res.status(400).json({ mensaje: "Verifique los datos obligatorios de la vacunación." });
    }

    if (proxima_dosis && proxima_dosis < fecha_aplicacion) {
        return res.status(400).json({ mensaje: "La próxima dosis no puede ser anterior a la fecha de aplicación." });
    }
    ...
    conexion.query(
        "SELECT id_vacunacion FROM vacunaciones WHERE id_mascota = ? AND id_vacuna = ? AND fecha_aplicacion = ? LIMIT 1",
        [id_mascota, id_vacuna, fecha_aplicacion],
        function(error, existentes) {
            if (error) { return res.status(500).json({ mensaje: "Error al validar el registro de vacunación" }); }
            if (existentes.length > 0) { return res.status(409).json({ mensaje: "La misma vacuna ya está registrada para esta mascota en esa fecha." }); }
